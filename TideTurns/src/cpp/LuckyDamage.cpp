...
LuckyDamage::LuckyDamage(EFModAPI *e, const nlohmann::json& j) {
    if (!j["LuckyDamage"]["enabled"]) return;
    API = e;
    old_DamageVar = nullptr;
    mode = j["LuckyDamage"]["mode"];
...
int LuckyDamage::DamageVar(BNM::UnityEngine::Object* i, float dmg, float luck) {

    if (old_DamageVar == nullptr) old_DamageVar = API->getAPI({"Assembly-CSharp.dll", "Terraria", "Main", "DamageVar", "old_fun"});
    std::random_device rd;
    std::mt19937 gen(rd());
    ...
    if (dis(gen) <= 50) {
        results = ((int(*)(BNM::UnityEngine::Object*, float, float)) old_DamageVar)(i, dmg, luck);
    }

    BNM_LOG_DEBUG("伤害值:%d", results);
